...
<script>
  ...
  const questions = [
    ...
    },
    {
      q: "你喜欢参与哪类活动？",
      options: [
        { text: "户外运动和挑战", scores: {Athletic:8, RiskTolerance:8} },  // Athletic维度本次不算，留空
  ...
  form.addEventListener('change', () => {
    const answeredCount = [...form.elements].filter(el => el.checked).length;
  ...
  function generateDetails(scores) {
    ...
    for (const key in scores) {
      ...
      lis += `<li><strong>${dimensionLabels[key]}</strong> (${level})：${descriptions[key]} 当前得分：${score}</li>`;
    }
    return lis;
  }
  ...
  function drawRadarChart(scores) {
    ...
    const dataLabels = Object.values(dimensionLabels);
    const dataScores = Object.keys(scores).map(k => scores[k]);
